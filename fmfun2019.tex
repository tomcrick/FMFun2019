...
\item[*]The authors note that many sites go for the second option, as it's easy to ``tick the box''. Apart from the fact that firewalls can be misconfigured, as in the Capital One attack \cite{Krebs2019s}, then can also be placed in ``detect-only'' mode and ignored\footnote{\cite{Kolochenko2016a}: ``Midsize and small companies frequently install WAFs just to satisfy a compliance requirement. They don't really care about practical security, and obviously won't care about maintaining their WAF. ''} 
\end{itemize}
\end{description}
